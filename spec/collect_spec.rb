...
describe SMG::Model, "#collect" do
  ...
  before :each do
    ...
    @data = File.read(FIXTURES_DIR + 'discogs/Genosha+Recordings.xml')
  end
  ...
  it "never overrides reader" do
    ...
    @klass.class_eval(<<-EOS, __FILE__, __LINE__ + 1)
      def urls
        ...
        @urls.select{|u| !u.empty?}.map{|u| u.strip}.compact.join(', ')
      end
    EOS
...
describe SMG::Model, "#collect", "without :class option" do
  ...
  it "handles characters and attributes independently" do
    custom_xml = <<-XML
    <releases>
  ...
  it "does nothing if there's no matching elements" do
    @klass.root 'resp/label'
...
describe SMG::Model, "#collect", "when :class option represents an SMG::Resource" do
  ...
  it "is able to build overlapped collections" do
    @klass.root 'resp/label'
    @klass.collect 'releases/release' , :as => :releases    , :class => @release_class
  ...
  it "does nothing when there's no matching elements" do
    @klass.root 'resp/label'
...
describe SMG::Model, "#collect", "when :class option represents a built-in typecast" do
...
describe SMG::Model, "#collect", "with nested collections" do
  ...
  before :each do
    @klass = Class.new { include SMG::Resource }
    @data = File.read(FIXTURES_DIR + 'discogs/948224.xml')
  end
  ...
  before :each do
    ...
    @klass.collect 'resp/release/tracklist/track', :as => :tracks, :class => @track_class
  end
  ...
  it "builds nested collections" do
  ...
  it "handles each collection independently" do
    ...
    @release.tracklist.should == [
      "Butterfly V.I.P. (VIP Edit By Ophidian)", 
      "Butterfly V.I.P. (Interlude By Cubist Boy & Tapage)",
      "Butterfly V.I.P. (Original Version)",
      "Hammerhead V.I.P. (VIP Edit By Ophidian)",
      "Hammerhead V.I.P. (Interlude By Cubist Boy & Tapage)",
      "Hammerhead V.I.P. (Original Version)" ]
